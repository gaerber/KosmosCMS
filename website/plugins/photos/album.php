	...
	if ($album_info = readAlbumConfig($ftp, $current_path)) {
		...
		if ($access['locked'] == 0) {
			...
			if (CheckAccess($access['access'])) {
				...
				$result = Database::instance()->query('SELECT id_str FROM '.DB_TABLE_PLUGIN.'photoalbum WHERE menu_sub='.$album_info['id'].'
				...
				while ($row = $result->fetch_assoc()) {
					...
					if ($album_info_sub = readAlbumConfig($ftp, $current_path.$row['id_str'].'/')) {
						...
						$res = Database::instance()->query('SELECT count(*) FROM '.DB_TABLE_PLUGIN.'photoalbum WHERE menu_sub='.$album_info_sub['id'])
							OR FatalError(FATAL_ERROR_MYSQL);
						if ($l = $res->fetch_row()) {
							$ctr_sub_albums = $l[0];
						}
						...
						$res = Database::instance()->query('SELECT count(*) FROM '.DB_TABLE_PLUGIN.'photoalbum_photo WHERE album_id='.$album_info_sub['id'])
							OR FatalError(FATAL_ERROR_MYSQL);
						if ($l = $res->fetch_row()) {
							$ctr_sub_photos = $l[0];
						}
						...
						if ($ctr_sub_photos > 0) {
							$res = Database::instance()->query('SELECT file_name FROM '.DB_TABLE_PLUGIN.'photoalbum_photo WHERE album_id='.$album_info_sub['id'].'
									ORDER BY RAND() LIMIT '.MODULE_PHOTOS_ALBUMPREVIEWPICS) OR FatalError(FATAL_ERROR_MYSQL);
							while ($r = $res->fetch_assoc()) {
								$arrayPreviewImg[] = array('folder' => $album_info_sub['id_str'].'/', 'img' => $r['file_name']);
							}
						}
						else if ($ctr_sub_albums > 0) {
							$res = Database::instance()->query('SELECT a.id, a.id_str, p.file_name FROM '.DB_TABLE_PLUGIN.'photoalbum AS a
				...
				/* Anzeige der Fotos */
				$result = Database::instance()->query('SELECT * FROM '.DB_TABLE_PLUGIN.'photoalbum_photo WHERE album_id='.$album_info['id'].' ORDER BY file_name ASC')
						OR FatalError(FATAL_ERROR_MYSQL);
				...
				while ($row = $result->fetch_assoc()) {
					...
					if ($ftp->fileExists($current_path.$row['file_name'])) {
						...
						Database::instance()->query('DELETE FROM '.DB_TABLE_PLUGIN.'photoalbum_photo WHERE id='.$row['id'].' AND album_id='.$album_info['id'])
								OR FatalError(FATAL_ERROR_MYSQL);
					}
				}
