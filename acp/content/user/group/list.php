...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
	...
	if (Database::instance()->query("UPDATE ".DB_TABLE_ROOT."cms_access_user
			SET user_access=(user_access & (~(1<<".StdSqlSafety($_GET['delete']).")))")) {
		/* Gruppe loeschen */
		if (Database::instance()->query("DELETE FROM ".DB_TABLE_ROOT."cms_access_group
			...
			echo ActionReport(REPORT_ERROR, "Fehler",
					"Die Gruppe konnte nicht gelöscht werden!<br />MySQL Fehler: ".Database::instance()->getErrorMessage());
		}
	}
	else {
		echo ActionReport(REPORT_ERROR, "Fehler",
...
if ($result->num_rows) {
	...
	while ($row = $result->fetch_assoc()) {
		...
		echo "      <td>".$row['name']."</td>\r\n";
		/* Anzahl Mitglieder */
		$res = Database::instance()->query("SELECT count(*) FROM ".DB_TABLE_ROOT."cms_access_user
